...
export default function ProceedDpProposalModal() {
    ...
    async function proposeHandler() {
        setIsLoading(true);
        try {
            const res = await backendService.proceedDp(price, assetidmanagement.data);
            console.log(res);
            setNotificationData({ title: "success to pay dp", description: `${res}`, position: "bottom-right" })
        } catch (error) {
            setNotificationData({ title: "failed to pay dp", description: "", position: "bottom-right" })
            console.log(error);
        } finally {
            setIsLoading(false);
            setModalKind(null);
            ...
            <div className="w-[80vw] md:w-[40vw] bg-white rounded-lg border border-gray-300 p-4">
                <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                        <h1 className="text-xl">Proceed Done Payment</h1>
                        <button
